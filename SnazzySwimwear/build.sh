#!/usr/bin/env bash
# Build the Snazzy Swimwear ONI mod: recolour kanims, compile C#, package, deploy to Dev.
set -e

ONI_MANAGED="$HOME/.local/share/Steam/steamapps/common/OxygenNotIncluded/OxygenNotIncluded_Data/Managed"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEV="$HOME/.config/unity3d/Klei/Oxygen Not Included/mods/Dev/SnazzySwimwear"
...
mcs \
  -target:library \
  -out:"$SCRIPT_DIR/dist/SnazzySwimwear.dll" \
  -optimize+ \
  -langversion:7.2 \
...
echo "==> Deploying to Dev/SnazzySwimwear ..."
rm -rf "$DEV"
mkdir -p "$DEV"
