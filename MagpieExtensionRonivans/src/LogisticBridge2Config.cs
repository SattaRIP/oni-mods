...
{
    public class LogisticBridge2Config : IBuildingConfig
    {
        public const string ID = "LogisticBridge2";
        public const string BASE_TYPE = "RonivansLegacy_ChemicalProcessing.Content.Defs.Buildings.DupesLogistics.LogisticBridgeConfig";
        private static readonly IBuildingConfig BaseConfig = RonivansHelpers.CreateBaseConfig(BASE_TYPE);

        public override BuildingDef CreateBuildingDef()
        {
            if (BaseConfig == null) return RonivansHelpers.CreateMissingDependencyDef(ID, 4, "logistic_bridge4_kanim");
            BuildingDef baseDef = BaseConfig.CreateBuildingDef();
            BuildingDef def = BuildingTemplates.CreateBuildingDef(
                ID, 4, 1, "logistic_bridge4_kanim", 30, 30f,
